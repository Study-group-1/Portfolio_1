...
```{r}
#Removing all punctuations in the Subject column  (Tobias)
...
```{r}
# Create dataframes with only columns to be used (Gustav)
...
```{r}
# Merge data frames (Magnus)
df <- merge(demo1, LU1, all = T)
df <- merge(df, token1, all = T)
```
...
```{r}
# Create data frame with only visit 1 (Morten)
visit1 <- subset(df, VISIT == 1)
visit1 <- visit1 %>% select(SUBJ, ADOS, MullenRaw, ExpressiveLangRaw, Socialization)

# Change variable names and merge with original data frame (Nicoline)
...
```{r}
# Changing 1 and 2 in the Gender column to 'F' for Female and 'M' for Male (Tobias)
df$Gender <- ifelse(df$Gender == 1, "M", "F")
df$Gender <- as.factor(df$Gender)

# Changing the values of Diagnosis from 'A' to 'ASD' and 'B' to 'TD' (Gustav)
...
# Changing name to anonymize participants (Magnus)
