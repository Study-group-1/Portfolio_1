...
```{r}
df_first <- merge(demo1, LU1)
df <- merge(df_first, token1)
```
...
#df_merge$Gender <- ifelse(df_merge$Gender==1, "F", "M")
#df_merge$Gender <-  as.factor(df_merge$Gender)
#df_merge$Diagnosis <- ifelse(df_merge$Diagnosis=="A", "ASD", "TD")
#df_merge$Diagnosis <-  as.factor(df_merge$Diagnosis)

      #Otherwise
            #df_merge$Gender <- mapvalues(df_merge$Diagnosis, from = c("A", "B"), to = c("ASD", "TD"))

#Changing name to anonymize participants
df_merge$SUBJ <- as.factor(df_merge$SUBJ)
df_merge$SUBJ <- as.numeric(df_merge$SUBJ)
df_merge$SUBJ <- as.factor(df_merge$SUBJ)
```
