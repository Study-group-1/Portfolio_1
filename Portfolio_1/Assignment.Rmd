...
author: "Tobias, Magnus, Morten, Gustav, Nicoline"
date: "03.09.20"
...
```{r}
pacman::p_load(tidyverse,janitor,plyr,stringr,dplyr)
```
...
```{r}
demo <- rename(demo, c("Child.ID"="SUBJ", "Visit"="VISIT"))
```
...
```{r}
#Removing all punctuations in the Subject column 
demo$SUBJ <- str_remove_all(demo$SUBJ, "\\.")
LU$SUBJ <- str_remove_all(LU$SUBJ, "\\.")
token$SUBJ <- str_remove_all(token$SUBJ, "\\.")
```
...
```{r}
#Merge first and then select the variables she's asking for? 

demo1 <- demo %>% select(SUBJ, VISIT, Diagnosis, Ethnicity, Gender, Age, ADOS, MullenRaw, ExpressiveLangRaw, Socialization)

LU1 <- LU %>% select(SUBJ, VISIT, MOT_MLU, CHI_MLU)

token1 <- token %>% select(SUBJ, VISIT, types_MOT, types_CHI, tokens_MOT, tokens_CHI)
```
...
visit1 <- subset(df, VISIT == 1)
visit1 <- visit1 %>% select(SUBJ, ADOS, MullenRaw, ExpressiveLangRaw, Socialization)
visit1 <- rename(visit1, c("ADOS" = "ADOS1", "MullenRaw" = "MullenRaw1", "ExpressiveLangRaw" = "ExpressiveLangRaw1", "Socialization" = "Socialization1"))
df <- merge(df, visit1)

#df_2f <- df %>% select(SUBJ, VISIT, ADOS, MullenRaw, ExpressiveLangRaw, Socialization)
#df_2f <- subset(df_2f, VISIT == 1)
#df_2f <- df_2f %>% select(SUBJ, ADOS, MullenRaw, ExpressiveLangRaw, Socialization)
#df_2f <- rename(df_2f, c("ADOS"="ADOS1", "MullenRaw"="MullenRaw1", "ExpressiveLangRaw"="ExpressiveLangRaw1", "Socialization"="Socialization1"))
#df_merge <- merge(df, df_2f)
```
...
```{r}
#Changing 1 and 2 in the Gender column to 'F' for Female and 'M' for Male

df$Gender <- ifelse(df$Gender == 1, "M", "F")
df$Gender <- as.factor(df$Gender)

df_merge$Gender <- as.character(df_merge$Gender)
df_merge$Gender[df_merge$Gender == "1"] <- "M"
df_merge$Gender[df_merge$Gender == "2"] <- "F"
df_merge$Gender <- as.factor(df_merge$Gender)

#Changing the values of Diagnosis from 'A' to 'ASD' and 'B' to 'TD'
df$Diagnosis <- ifelse(df$Diagnosis=="A", "ASD", "TD")
df$Diagnosis <-  as.factor(df$Diagnosis)


df_merge$Diagnosis <- as.character(df_merge$Diagnosis)
df_merge$Diagnosis[df_merge$Diagnosis == "A"] <- "ASD"
df_merge$Diagnosis[df_merge$Diagnosis == "B"] <- "TD"
df_merge$Diagnosis <- as.factor(df_merge$Diagnosis)

#Another code to do the same as we did above 
#df$Gender <- mapvalues(df$Gender, from = c("1", "2"), to = c("M", "F"))
#df$Diagnosis <-  mapvalues(df$Diagnosis, from = c("A", "B"), to = c("ASD", "TD"))

#Changing name to anonymize participants
